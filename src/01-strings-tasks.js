 ...
 *   'ABABAB','BA' => 'ABAB'
 */
function removeFirstOccurrences(str, occ) {
...
function getRectangleString(width, height) {
  ...
  const square = start + y.repeat(height - 2) + end;
  return square;
}
...
function encodeToRot13(str) {
  ...
  return str.split('').map((elem) => (elem.match(/\W/) ? elem : output.slice(input.indexOf(elem), input.indexOf(elem) + 1))).join('');
}
